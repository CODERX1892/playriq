...
Keep it under 350 words. Be direct. This is a performance review not a pep talk.`
  ...
  return (
    ...
    const advPassPerGame = r1(stats.reduce((s,r) => s + n(r.advance_pass), 0) / mc)
    const tacklesPerGame = r1(stats.reduce((s,r) => s + n(r.tackles), 0) / mc)
    const forcedTOPerGame = r1(stats.reduce((s,r) => s + n(r.forced_to_win), 0) / mc)
    const kickawayTOWonPerGame = r1(stats.reduce((s,r) => s + n(r.kickaway_to_received), 0) / mc)

    const isForward = ['Inside Forward','Half Forward'].includes(role)
    const isBack = ['Full Back','Half Back'].includes(role)
    const isMid = role === 'Midfielder'
...
- Simple passes per game: \${simplePassPerGame}
- Advance passes per game: \${advPassPerGame}
- Carries per game: \${carriesPerGame}
- Turnovers in contact: \${r1(stats.reduce((s,r)=>s+n(r.turnovers_in_contact),0)/mc)}/game (target: zero)
- Kickaway turnovers: \${r1(stats.reduce((s,r)=>s+n(r.turnovers_kicked_away),0)/mc)}/game (target: zero)
- Skill errors: \${r1(stats.reduce((s,r)=>s+n(r.turnover_skill_error),0)/mc)}/game (target: zero)
- Drop shorts: \${r1(stats.reduce((s,r)=>s+n(r.drop_shorts),0)/mc)}/game (target: zero)
...
DEFENSIVE WORKRATE:
- Tackles per game: \${tacklesPerGame}\${isForward ? ' (forwards should contribute 1-2/game)' : ' (backs/mids should contribute 3+/game)'}
\${!isForward ? \`- Forced turnovers won: \${forcedTOPerGame}/game
- Kickaway turnovers won: \${kickawayTOWonPerGame}/game\` : \`- Forced turnovers won: \${forcedTOPerGame}/game\`}
...
DEFENCE  
Cover tackles, \${isForward ? 'and pressure on opposition — forwards contribute to defence through tackle count and press' : 'forced turnovers, defensive actions and any kickout involvement'}.

TARGETS FOR NEXT GAME
3 specific, measurable targets — one for each section above. Set them as realistic steps up from current performance, not aspirational jumps. State the number to aim for.

Keep it under 380 words. Plain section headers only. Constructive and actionable throughout.\`pointer_attempts: { min: 2.0, good: 3.0, p90: 3.0, lower_is_better: false, zero_target: false, warning: 2.0, label: "Target 3.0+ per game" },
    pts: { min: 2.0, good: 2.5, p90: 3.0, lower_is_better: false, zero_target: false, warning: 2.0, label: "Target 2.5+ per game" },
    duels_contested: { min: 1.0, good: 1.0, p90: 1.0, lower_is_better: false, zero_target: false, warning: 1.0, label: "Target 1.0+ per game" },
    tackles: { min: 3.0, good: 3.0, p90: 5.0, lower_is_better: false, zero_target: false, warning: 3.0, label: "Target 3.0+ per game" },
    forced_to_win: { min: 1.0, good: 1.0, p90: 1.0, lower_is_better: false, zero_target: false, warning: 1.0, label: "Target 1.0+ per game" },
